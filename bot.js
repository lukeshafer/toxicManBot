...
const stopEric = (message) => {
  let newMsg = message.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  const checkStrings = [
    'viet',
    'nam',
    'tnma',
  ...
  const didHeSayIt = checkStrings.some((check) => {
    return newMsg.includes(check) || message.includes('🇻🇳');
  });
  return didHeSayIt;
};
  ...
  if (message.substring(0, 1) == '!') {
    ...
    switch (cmd) {
      ...
      case 'roll':
        ...
        send_message(
          channelID,
          "jesus fucking christ pal, how'd you learn about this???"
        );
        break;
      case 'ask':
      case 'question':
      ...
      case 'addCommand': // Add new commands with static responses
        ...
        if (!checkIfOp(userID)) {
          ...
          send_message(
            channelID,
            "thaAT's a BAD COmmand!!!!! We HAVVE IT ALREADY"
          );
        }
        break;
    ...
    const response = responses[getRandomInt(responses.length)];
    send_message(channelID, response);
  } else if (evt.d.hasOwnProperty('member') && !message.includes('||')) {
    // respond if ANY message contains a specific word in it
    messageText = '';
